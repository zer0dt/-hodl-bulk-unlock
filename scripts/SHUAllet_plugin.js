...
const isTimeLockScript = (script) => {
    try {
        const scriptObj = bsv.Script(script);
        // Check if this matches our time-lock script pattern
        return scriptObj.chunks.length > 6 && scriptObj.chunks[6].buf;
    } catch(e) {
        return false;
    }
}

const isOutputSpent = async (txid, vout = 0) => {
    try {
        const url = `https://api.bitails.io/tx/${txid}/output/${vout}`;
        const response = await fetch(url);
        const data = await response.json();
        return data.spent; // Bitails API returns a "spent" boolean
    } catch (error) {
        console.log(`Error checking if output is spent: ${error.message}`);
        return null;
    }
}
...
const unlockCoins = async(pkWIF, receiveAddress, txid, oIdx = 0) => {
    try {
        // First check if output is already spent
        const isSpent = await isOutputSpent(txid, oIdx);
        if (isSpent) {
            console.log(`Skipping ${txid}: Output already spent`);
            return null;
        }
        ...
        if (!isTimeLockScript(lockedUTXO.script)) {
            console.log(`Skipping ${txid}: Not a time-locked transaction`);
            return null;
        }
        ...
        bsvtx.to(receiveAddress, lockedUTXO.satoshis === 1 ? 1 : lockedUTXO.satoshis - 1);
        ...
        console.log(`Error unlocking coins for ${txid}: ${e.message}`);
        console.log(e);
        return null;
    }
}
const bulkUnlock = async(pkWIF, receiveAddress, identityAddress) => {
    console.log(`Starting bulk unlock process...WOC`);
    log(`Fetching transaction history from WhatsOnChain!...`);
    let hasMore = true;
    let token = '';
    let processedCount = 0;
    const startHeight = 821149;
    
    while (hasMore) {
        try {
            const baseUrl = `https://api.whatsonchain.com/v1/bsv/main/address/${identityAddress}/confirmed/history`;
            ...
            log(`Fetching transactions batch...`);
            ...
            log(`Processing batch of ${res.result.length} transactions`);
            
            for (let i = 0; i < res.result.length; i++) {
                const tx = res.result[i];
                processedCount++;
                log(`\nTransaction ${processedCount}: ${tx.tx_hash}`);
                log(`Block Height: ${tx.height}`);
                
                try {
                    // Check if output is spent
                    const isSpent = await isOutputSpent(tx.tx_hash);
                    if (isSpent) {
                        log(`Status: Already spent`);
                        continue;
                    }

                    const rawtx = await unlockCoins(pkWIF, receiveAddress, tx.tx_hash);
                    if (rawtx) {
                        log(`Status: Generated unlock transaction`);
                        
                        const broadcastResult = await broadcast(rawtx);
                        log(`Status: Successfully unlocked - ${JSON.stringify(broadcastResult)}`);
                    } else {
                        log(`Status: Skipped - Not a time-locked transaction`);
                    }
                    
                    // Add a small delay between transactions to avoid rate limiting
                    if (i < res.result.length - 1) {
                        log('Waiting 1 second before next transaction...');
                        await sleep(1000);
                    }
                } catch (error) {
                    log(`Status: Error - ${error.message || 'Unable to unlock'}`);
                }
            }
            ...
            if (res.token) {
                token = res.token;
                log(`\nMoving to next page with token: ${token}`);
                await sleep(1000); // Add delay between page requests
            } else {
                hasMore = false;
